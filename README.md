...
- **調査期間指定** (v7.3): サイドバーで開始年+月を任意設定（デフォルト: 直近1年）
...
### 3. 店舗・教室調査
- **AI 2段階チェック** (v7.3): Gemini AI → 要確認時に Perplexity で補助検証
- **2段階ブランド発見** (v7.2): 0件時に企業名→ブランド名特定→再調査を自動実行
- **都道府県別店舗・教室数**: 概算数値で出力（v7.1、旧 有無判定 互換）
...
│   ├── constants.py             # バージョン・User-Agent定数 (v7.2)
│   ├── robots_checker.py        # robots.txt自動準拠チェッカー (v7.2)
│   ├── rate_limiter.py          # ドメイン別レート制限 (v7.2)
│   ├── request_audit.py         # HTTPリクエスト監査ログ (v7.2)
...
│   └── app_logger.py            # StreamlitデバッグログUI (v7.3)
│
...
| Ver | 日付 | 変更内容 |
|-----|------|---------|
| **v7.1** | 2026-03-10 | 現場FB対応: 全investigator動的期間制約、店舗調査ブランド展開検出、都道府県別店舗数（数値化）、テスト467件 |
