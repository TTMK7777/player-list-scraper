...
class NewcomerDetector:
    ...
    def estimate_cost() -> dict:
        ...
        return {
            "call_count": 1,
            "cost_per_call": NewcomerDetector.COST_PER_CALL,
            "estimated_cost": NewcomerDetector.COST_PER_CALL,
        }
    ...
    async def detect(
        ...
        # Step 1: LLMに問い合わせ
        candidates = await self._query_newcomers(industry, existing_players, definition=definition, start_year=start_year, start_month=start_month)
...
【時間スコープ】
- 「新規参入」= {period_label}にサービスを開始したプレイヤー
    ...
    def _parse_response(self, raw_response: str) -> list[NewcomerCandidate]:
        ...
        for item in candidates_data:
            ...
            candidate = NewcomerCandidate(
                ...
                entry_date_approx=parsed.entry_date_approx,
